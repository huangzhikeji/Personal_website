export async function onRequest({ request, env }) {
    ...
    if (request.method === 'POST') {
        try {
            ...
            if (logo !== null) { 
                if (logo && logo !== '/img/logo.png') { await NAV_KV.put('site_logo', logo); } 
                else { await NAV_KV.delete('site_logo'); }
            }
            if (logoLink !== null) { if (logoLink) await NAV_KV.put('site_logo_link', logoLink); else await NAV_KV.delete('site_logo_link'); }
            if (headerBg !== null) { 
                if (headerBg && headerBg !== '/img/bg.jpg') { await NAV_KV.put('header_bg', headerBg); } 
                else { await NAV_KV.delete('header_bg'); }
            }
