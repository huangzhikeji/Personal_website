// functions/api/images.js - 完整版（含清理未使用图片按钮）
...
<h3>危险操作区</h3>
<p>清空所有KV数据将删除：文章、书签、图片、站点设置、管理员密码等所有数据！</p>
...
document.getElementById('clearAllDataBtn').onclick=async()=>{if(!confirm('清空所有KV数据？输入确认删除'))return;if(prompt('请输入确认删除')!=='确认删除')return;var res=await fetch('/api/images?clear=1',{method:'DELETE'});var data=await res.json();alert(data.message);loadImages();};
loadImages();
</script>
</body>
...
async function updateImageList(env) {
    // 辅助函数，保持列表同步
}
