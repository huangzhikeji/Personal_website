...
<body>
    ...
    <div class="main">
        <div class="header">${headerBg ? `<img class="header-bg-img" src="${escapeHtml(headerBg)}">` : ''}<div class="header-content"><h1>${escapeHtml(siteTitle || '旭儿导航可自定义')}</h1>${cnLink ? `<a href="${escapeHtml(cnLink)}" class="cn-btn" target="_blank">点击前往国外服务器</a>` : ''}<p>${escapeHtml(siteSubtitle || '精选网站 · 优质博客可自定义')}</p><div>📅 ${new Date().toLocaleDateString('zh-CN')}</div></div></div>
        <div class="content">
