...
<button type="submit">登录</button></form></div></body></html>`, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
    ...
    // 读取数据
    let sites = [], posts = [];
    let siteTitle = await NAV_KV.get('site_title') || '';
    let siteSubtitle = await NAV_KV.get('site_subtitle') || '';
    let cnLink = await NAV_KV.get('cn_link') || '';
...
.settings-form{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;}
...
<body><div class="container">
    ...
    <!-- 站点设置卡片 -->
    <div class="card"><h2>⚙️ 站点设置</h2>
        <div class="settings-form">
            <input type="text" id="siteTitle" placeholder="站点标题" value="${escapeHtml(siteTitle)}">
            <input type="text" id="siteSubtitle" placeholder="站点副标题" value="${escapeHtml(siteSubtitle)}">
            <input type="url" id="cnLink" placeholder="国内线路链接" value="${escapeHtml(cnLink)}">
...
async function saveSettings() {
    ...
    const cnLink = document.getElementById('cnLink').value.trim();
